...
#case for functions, dble only support CURRENT_TIME() but not support CURRENT_TIME without brackets, this is a known-limit. ATK-1249
    Given execute single sql in "dble-1" in "user" mode and save resultset in "91"
      | conn   | toClose | sql                          | db      | charset |
      | conn_0 | False   | SELECT t1.name, t1.dim1,ROUND(IFNULL(t2.dataValue,0),2) dataValue,'件' unit FROM(SELECT 'AUTO' dim1,'自动成功数' name UNION ALL SELECT 'REMOTE' dim1,'远程成功数' name UNION ALL SELECT 'PLAT' dim1,'掌机成功数' name UNION ALL SELECT 'REINPUT' dim1,'回录成功数' name UNION ALL SELECT 'FAILED' dim1,'失败数' name UNION ALL SELECT 'DOING' dim1,'执行中数' name UNION ALL SELECT 'WAITING' dim1,'待执行数' name)t1 LEFT JOIN (SELECT dim1, sum(DATA_VALUE) dataValue FROM CL_IDX_DATA_MONITOR A WHERE A.IDX_NO = 'JYGK41101060000002001' AND STAT_TIME = CONCAT(':statTime',left(CURRENT_TIME(),2),'00') AND ORG_NO = ':orgCode' group by dim1)t2 ON t1.dim1=t2.dim1; | schema1 | utf8mb4 |
    Then check resultset "91" has lines with following column values
      | name-0    | dim1-1 | dataValue-2 | unit-3 |
      | 自动成功数 | AUTO    | 0.00        | 件     |
      | 远程成功数 | REMOTE  | 0.00        | 件     |
      | 掌机成功数 | PLAT    | 0.00        | 件     |
      | 回录成功数 | REINPUT | 0.00        | 件     |
      | 失败数    | FAILED  | 0.00         | 件    |
      | 执行中数  | DOING    | 0.00        | 件     |
      | 待执行数  | WAITING  | 0.00        | 件     |


#case for fix "column name not found". ATK-1231
    Then execute sql in "dble-1" in "user" mode
      | conn   | toClose  | sql                                        | expect       | db      | charset |
      | conn_0 | False    | SELECT SUM(CASE t.name WHEN "网上国网" THEN t.dataValue END) "wsgw",       s.DESCRIPTION "dw" FROM ( SELECT "网上国网" AS name, ROUND(IFNULL(SUM(`DATA_VALUE`),0),0) dataValue FROM cl_idx_data_monitor cidm WHERE cidm.ORG_NO = '41101' AND cidm.IDX_NO = 'JYGK41101040000000001' AND cidm.STAT_TIME >= '20200802' AND cidm.STAT_TIME <= '20200803' AND cidm.STAT_CALIBRE = '01' AND dim1 ='05' UNION ALL SELECT "支付宝" AS "name", ROUND(IFNULL(SUM(`DATA_VALUE`),0),0) dataValue FROM cl_idx_data_monitor cidm WHERE cidm.ORG_NO = '41101' AND cidm.IDX_NO = 'JYGK41101040000000001' AND cidm.STAT_TIME >= '20200802' AND cidm.STAT_TIME <= '20200803' AND cidm.STAT_CALIBRE = '01' AND dim1 = "04"  UNION ALL SELECT "电e宝" AS "name", ROUND(IFNULL(SUM(`DATA_VALUE`),0),0) dataValue FROM cl_idx_data_monitor cidm WHERE cidm.ORG_NO = '41101' AND cidm.IDX_NO = 'JYGK41101040000000001' AND cidm.STAT_TIME >= '20200802' AND cidm.STAT_TIME <= '20200803' AND cidm.STAT_CALIBRE = '01' AND dim1 = "01" ) t LEFT JOIN sys_dict_entry s ON s.`CODE` = '41101'| has{(0,NULL)}       | schema1 | utf8mb4 |
      | conn_0 | False    | SELECT SUM(CASE t.name WHEN "网上国网" THEN t.dataValue END) "wsgw",       s.DESCRIPTION "dw" FROM ( SELECT ROUND(IFNULL(SUM(`DATA_VALUE`),0),0) dataValue , "网上国网" AS name   FROM cl_idx_data_monitor cidm WHERE cidm.ORG_NO = '41101' AND cidm.IDX_NO = 'JYGK41101040000000001' AND cidm.STAT_TIME >= '20200802' AND cidm.STAT_TIME <= '20200803' AND cidm.STAT_CALIBRE = '01' AND dim1 ='05' UNION ALL SELECT ROUND(IFNULL(SUM(`DATA_VALUE`),0),0) dataValue,"支付宝" AS "name" FROM cl_idx_data_monitor cidm WHERE cidm.ORG_NO = '41101' AND cidm.IDX_NO = 'JYGK41101040000000001' AND cidm.STAT_TIME >= '20200802' AND cidm.STAT_TIME <= '20200803' AND cidm.STAT_CALIBRE = '01' AND dim1 = "04" UNION ALL SELECT ROUND(IFNULL(SUM(`DATA_VALUE`),0),0) dataValue,"电e宝" AS "name" FROM cl_idx_data_monitor cidm WHERE cidm.ORG_NO = '41101' AND cidm.IDX_NO = 'JYGK41101040000000001' AND cidm.STAT_TIME >= '20200802' AND cidm.STAT_TIME <= '20200803' AND cidm.STAT_CALIBRE = '01' AND dim1 = "01" ) t LEFT JOIN sys_dict_entry s ON s.`CODE` = '41101' | has{(0,NULL)}     | schema1 | utf8mb4 |
      | conn_0 | true     | SELECT SUM(CASE t.name WHEN "网上国网" THEN t.dataValue END) "wsgw",       s.DESCRIPTION "dw" FROM ( SELECT "网上国网" AS name, ROUND(IFNULL(SUM(`DATA_VALUE`),0),0) dataValue FROM cl_idx_data_monitor cidm WHERE cidm.ORG_NO = '41101' AND cidm.IDX_NO = 'JYGK41101040000000001' AND cidm.STAT_TIME >= '20200802' AND cidm.STAT_TIME <= '20200803' AND cidm.STAT_CALIBRE = '01' AND dim1 ='05' UNION ALL SELECT "支付宝" AS "name", ROUND(IFNULL(SUM(`DATA_VALUE`),0),0) dataValue FROM cl_idx_data_monitor cidm WHERE cidm.ORG_NO = '41101' AND cidm.IDX_NO = 'JYGK41101040000000001' AND cidm.STAT_TIME >= '20200802' AND cidm.STAT_TIME <= '20200803' AND cidm.STAT_CALIBRE = '01' AND dim1 = "04"  ) t LEFT JOIN sys_dict_entry s ON s.`CODE` = '41101'| has{(0,NULL)}      | schema1 | utf8mb4 |
      | conn_0 | true     | SELECT SUM(CASE t.name WHEN "网上国网" THEN t.dataValue END) "wsgw",       s.DESCRIPTION "dw" FROM ( SELECT ROUND(IFNULL(SUM(`DATA_VALUE`),0),0) dataValue , "网上国网" AS name   FROM cl_idx_data_monitor cidm WHERE cidm.ORG_NO = '41101' AND cidm.IDX_NO = 'JYGK41101040000000001' AND cidm.STAT_TIME >= '20200802' AND cidm.STAT_TIME <= '20200803' AND cidm.STAT_CALIBRE = '01' AND dim1 ='05' UNION ALL SELECT ROUND(IFNULL(SUM(`DATA_VALUE`),0),0) dataValue,"支付宝" AS "name" FROM cl_idx_data_monitor cidm WHERE cidm.ORG_NO = '41101' AND cidm.IDX_NO = 'JYGK41101040000000001' AND cidm.STAT_TIME >= '20200802' AND cidm.STAT_TIME <= '20200803' AND cidm.STAT_CALIBRE = '01' AND dim1 = "04"  ) t LEFT JOIN sys_dict_entry s ON s.`CODE` = '41101' | has{(0,NULL)}   | schema1 | utf8mb4 |
